...
class Update:
    ...
    def __init__(self, root):
        ...
        self.root.title("Student Updation Console - FacI/^O")
        ...
        # Header Label
        header_text_label = Label(self.root, text="Update Student Details", bg="white", fg="blue",
        ...
        searchBtn = Button(sms_main_frame, text="Search", width=10, fg="white", bg="green",
                           command=self.search_for_roll)
        ...
        # Buttons Creation
        updateBtn = Button(left_frame_buttons, cursor="hand2", text="Update", width=14, fg="white", bg="green",
                           command=self.update_data)
        updateBtn.grid(row=0, column=0, padx=5, pady=10)
    ...
    def update_data(self):
        if self.studentname.get() == "" or self.studentroll.get() == "" or self.mobile.get() == "" or self.classname.get() == "":
            ...
            try:
                ...
                my_cursor = conn.cursor()
                my_cursor.execute(
                    f"update {self.classname.get().upper()} set roll=%s, name=%s, mobile=%s, class=%s where roll= %s", (
                ...
                conn.commit()
                conn.close()
                update_via = {"Rollno": self.searchableroll.get()}
                update_to = {"$set": {"Name": self.studentname.get().upper(),
                                      "mobile": self.mobile.get()}}
                self.Encoding[self.searchableroll.get().upper()].update_many(update_via, update_to)
